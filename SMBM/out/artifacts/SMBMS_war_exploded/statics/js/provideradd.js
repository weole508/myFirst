...
$(function(){
	...
	proCode.bind("blur",function(){
		...
		$.ajax({
			type:"GET",//请求类型
			url:path+"/sys/provider/proCode.json",//请求的url
			data:{proCode:proCode.val()},//请求参数
			dataType:"json",//ajax接口（请求url）返回的数据类型
			success:function(data){//data：返回数据（json对象）
				if(data.proCode == "exist"){//账号已存在，错误提示
					validateTip(proCode.next(),{"color":"red"},imgNo+ " 该供应商账号已存在",false);
				}else{//账号可用，正确提示
					validateTip(proCode.next(),{"color":"green"},imgYes+" 该供应商账号可以使用",true);
				}
			},
