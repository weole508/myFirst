...
<div class="right">
       ...
       <form method="post" action="${pageContext.request.contextPath }/sys/bill/list.html">
			<span>商品名称：</span>
			<input name="strQueryProductName" type="text" value="${strQueryProductName }">
			<span>供应商：</span>
			<select name="strQueryProviderId">
				<c:if test="${providerList != null }">
				   <option value="">--请选择--</option>
				   <c:forEach var="provider" items="${providerList}">
				   		<option <c:if test="${provider.id == strQueryProviderId }">selected="selected"</c:if>
				   		value="${provider.id}">${provider.proName}</option>
				   </c:forEach>
				</c:if>
       		</select>
			<span>是否付款：</span>
			<select name="strIsPayment">
				<option value="">--请选择--</option>
